...
export default function TripPlanner() {
  ...
  const generate = async () => {
    ...
    const prompt = `You are SportsTripz AI, an expert travel planner for Pacific Island sports teams. A coach needs a complete trip plan.
...
ROUTING
Recommend the best flight routing from the Pacific Island to the destination. Include stopovers, airlines, and estimated total travel time.
...
List all documents the coach needs: for athletes, for visa applications, for sports federation, for ministry funding.

FUNDING APPLICATION POINTS
3-4 key talking points for applying to the sports ministry or national Olympic committee for funding for this trip.

TIMELINE
Working backwards from ${form.month}, when should the coach start each preparation step.

Be specific, practical, and Pacific-focused. Use your knowledge of Samoan, Tongan, Fijian and other Pacific passport restrictions.`;
        ...
        <div style={styles.card}>
          <div style={styles.grid}>
            ...
            <div>
              ...
              <select style={styles.select} value={form.passport} onChange={e => setForm({...form, passport: e.target.value})}>
                <option value="">Select passport...</option>
                {PASSPORTS.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div>
